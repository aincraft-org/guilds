...
/** Async tax seam backed by the serialized guild-bank coordinator. */
public final class MintGuildTaxSettlement implements AsyncTaxSettlement {
    ...
    @Override public CompletionStage<AsyncSettlementResult> settle(UUID payerId, String guildId, BigDecimal amount, String key) {
        ...
        return bank.creditTax(payerId, guildId, amount, key).thenApply(result -> switch (result.status()) {
            ...
            case UNAVAILABLE, CAPACITY_EXCEEDED -> new AsyncSettlementResult(AsyncSettlementResult.Status.UNAVAILABLE, result.diagnosticCode(), result.receiptIdentifier());
            case UNAUTHORIZED, REJECTED -> new AsyncSettlementResult(AsyncSettlementResult.Status.REJECTED, result.diagnosticCode(), result.receiptIdentifier());
        });
    }
}
