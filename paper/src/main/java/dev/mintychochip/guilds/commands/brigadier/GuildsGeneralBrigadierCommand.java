...
public class GuildsGeneralBrigadierCommand {
    ...
    public LiteralCommandNode<CommandSourceStack> buildCommand() {
        return Commands.literal("guilds")
            .requires(source -> source.getSender().hasPermission("guilds.general")
                    || source.getSender().hasPermission("guilds.territory.building.manage")
    ...
    private com.mojang.brigadier.tree.LiteralCommandNode<CommandSourceStack> buildingNode() {
        if (buildings != null) {
    ...
    private int showHelp(CommandContext<CommandSourceStack> ctx) {
        ...
        sender.sendMessage("§6║                                                ║");
        sender.sendMessage("§6║ §f/guilds building§7                            §6║");
        sender.sendMessage("§6║   §8» Register guild buildings in a region     §6║");
        sender.sendMessage("§6╚══════════════════════════════════════════════╝");
        return Command.SINGLE_SUCCESS;
    }
