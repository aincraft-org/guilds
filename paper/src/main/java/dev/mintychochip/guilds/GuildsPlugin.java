...
import dev.mintychochip.territory.persist.PostgresGuildStorageStore;
...
public final class GuildsPlugin extends JavaPlugin {
    ...
    private dev.mintychochip.guilds.commands.brigadier.BuildingCommand buildingCommand;
    /** The legacy territory building command. */
    private dev.mintychochip.territory.building.BuildingCommand territoryBuildingCommand;
    /** The facility mutations. */
    private dev.mintychochip.territory.building.FacilityMutationService facilityMutations;
    /** The waystone travel service. */
    private dev.mintychochip.territory.building.WaystoneTravelService waystoneTravelService;
    /** The guild item-storage engine. */
    private dev.mintychochip.territory.storage.GuildStorageEngine guildStorageEngine;
    /** The store. */
    ...
    public void onEnable() {
        ...
        mintClientReceiver = new MintClientReceiver() {
            /**
             * Performs the bind mint client operation.
             * @param lease the lease
             */
            @Override public void bindMintClient(MintClientLease lease) {
                GuildsPlugin.this.bindMintClient(lease);
            }
        };
        ...
        try {
            ...
            this.economyBridge = new EconomyBridge(
                    registry, governance, dev.mintychochip.territory.economy.GoodsCatalog.defaultCatalog(),
    ...
    /** Performs the start upkeep operation. */
    ...
    /** Performs the tick upkeep operation. */
    ...
    /** Performs the stop upkeep operation. */
    ...
    /** Performs the stop influence status operation. */
    ...
    /**
     * Performs the development level for operation.
    ...
    /**
     * Performs the days to millis operation.
     * @param days the days
     * @return the result
     */
    ...
    /** Performs the start web if enabled operation. */
    ...
    /** Performs the stop web operation. */
    ...
    /** Performs the stop squaremap operation. */
    ...
    /** Performs the enable guilds subsystem operation. */
    ...
    /** Performs the disable guilds subsystem operation. */
    ...
    /** Performs the wire invasions operation. */
    ...
    /**
     * Performs the broadcast flips operation.
     * @param flips the flips
     */
    ...
    /**
     * Performs the resolve guild name operation.
    ...
    /**
     * Performs the resolve guild name for operation.
     * @param guildId the guild id
     * @return the result
     */
    ...
    /**
     * Returns the building command.
     * @return the result
     */
    public dev.mintychochip.guilds.commands.brigadier.BuildingCommand getBuildingCommand() {
        return buildingCommand;
    }
    /**
     * Returns the legacy territory building command.
     * @return the result
     */
    public dev.mintychochip.territory.building.BuildingCommand getTerritoryBuildingCommand() {
        return territoryBuildingCommand;
    }

    /**
     * Returns the facility mutations.
     * @return the result
     */
    ...
    /**
     * Returns the waystone travel service.
     * @return the result
     */
    ...
    /** Performs the start buildings operation. */
    private void startBuildings() {
        ...
        try {
            ...
            this.guildStorageEngine = new dev.mintychochip.territory.storage.GuildStorageEngine(
                    registry, facilities,
                    new dev.mintychochip.territory.storage.GuildPermissionStorageAccess(
                            guilds.getGuildService()),
                    new PostgresGuildStorageStore(database));
            var storageGui = new dev.mintychochip.territory.storage.GuildStorageGui(
                    this, guildStorageEngine, new dev.mintychochip.territory.storage.PaperItemCodec());
            this.buildingCommand = new dev.mintychochip.guilds.commands.brigadier.BuildingCommand(
                    sessions, facilities, registry, anchors, authorization,
                    facilityMutations, config, selections, waystoneTravelService);
            this.territoryBuildingCommand = new dev.mintychochip.territory.building.BuildingCommand(
                    sessions, facilities, registry, anchors, authorization,
                    facilityMutations, config, selections, waystoneTravelService);
            getServer().getPluginManager().registerEvents(
                    new dev.mintychochip.territory.building.BuildingListener(
                            ...
                            getServer().getPluginManager(), storageGui), this);
            getServer().getPluginManager().registerEvents(
            ...
            if (guilds.getGuildBrigadierCommand() != null) {
                guilds.getGuildBrigadierCommand().setStorage(storageGui);
                guilds.getGuildBrigadierCommand().setBuildingCommand(this.buildingCommand);
            }
            if (guilds.getGuildsGeneralCommand() != null) {
                guilds.getGuildsGeneralCommand().setBuildingCommand(this.buildingCommand);
            }
            getLogger().info("Territory anchor buildings enabled");
        } catch (RuntimeException e) {
            this.buildingCommand = null;
            this.territoryBuildingCommand = null;
            this.facilityMutations = null;
            this.waystoneTravelService = null;
            this.guildStorageEngine = null;
    ...
    /**
     * Returns the guild item-storage engine.
     *
     * @return engine, or {@code null} when buildings failed to start
     */
    public dev.mintychochip.territory.storage.GuildStorageEngine getGuildStorageEngine() {
        return guildStorageEngine;
    }

    /**
     * Returns the bukkit economy bridge.
     * @return the result
     */
