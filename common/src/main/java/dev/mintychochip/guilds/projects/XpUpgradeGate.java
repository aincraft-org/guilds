...
public final class XpUpgradeGate {
    ...
    /**
     * Subtracts one level's XP and keeps leftover progress.
     *
     * @param progress current upgrade progress
     * @param requiredExperience XP consumed by the level
     * @return remaining progress; empty when nothing is left
     */
    public static Map<String, Integer> consumeExperience(
            Map<String, Integer> progress, int requiredExperience) {
        int remainingXp = Math.max(0, contributedExperience(progress) - Math.max(0, requiredExperience));
        Map<String, Integer> leftover = new HashMap<>();
        if (progress != null) {
            for (Map.Entry<String, Integer> entry : progress.entrySet()) {
                if (!isExperienceKey(entry.getKey()) && entry.getValue() != null && entry.getValue() > 0) {
                    leftover.put(entry.getKey(), entry.getValue());
                }
            }
        }
        if (remainingXp > 0) {
            leftover.put(EXPERIENCE_KEY, remainingXp);
        }
        return leftover;
    }
