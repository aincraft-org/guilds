...
/**
 * Enum-based permission system for Towny
...
public enum TownyPermission {
    ...
    TownyPermission(String displayName, String description, Category category, int legacyBitwiseValue, int sortOrder) {
    ...
    /**
     * Convert legacy bitwise value to TownyPermission enum
     * @param legacyValue The legacy bitwise value
     * @return Optional containing the matching permission, or empty if not found
     */
    public static Optional<TownyPermission> fromLegacyValue(int legacyValue) {
    ...
    /**
     * Convert legacy bitwise flags to a set of TownyPermission enums
     * @param legacyFlags The legacy bitwise combination
     * @return Set of TownyPermission enums
     */
    public static Set<TownyPermission> fromLegacyFlags(int legacyFlags) {
        return Arrays.stream(values())
    ...
    /**
     * Convert a set of TownyPermission enums back to legacy bitwise flags
     * @param permissions The set of permissions
     * @return Legacy bitwise flags
     */
    public static int toLegacyFlags(Set<TownyPermission> permissions) {
        return permissions.stream()
                .mapToInt(TownyPermission::getLegacyBitwiseValue)
                .reduce(0, (a, b) -> a | b);
    }
     ...
     */
    public static List<TownyPermission> getByCategory(Category category) {
        return Arrays.stream(values())
                .filter(permission -> permission.category == category)
                .sorted(Comparator.comparingInt(TownyPermission::getSortOrder))
     ...
     */
    public static List<TownyPermission> getAllSorted() {
        return Arrays.stream(values())
                .sorted(Comparator
                        .comparing((TownyPermission p) -> p.category.getDisplayName())
                        .thenComparingInt(TownyPermission::getSortOrder))
                .collect(Collectors.toList());
    }
