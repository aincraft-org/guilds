...
Result: BUILD SUCCESSFUL. Six actionable tasks were evaluated; selected PostgreSQL invasion and database tests passed (integration tests are environment-assumption based when `GUILDS_TEST_JDBC_URL` is unavailable).

## Changed files

- `common/src/main/java/com/guilds/territory/invasion/PostgresInvasionStore.java`
...
- `common/src/main/java/com/guilds/territory/persist/PostgresDatabase.java`
...
- Focused command: `./gradlew :common:test --tests '*PostgresInvasionStoreTest'` — BUILD SUCCESSFUL (PostgreSQL integration tests are environment-assumption based when `GUILDS_TEST_JDBC_URL` is unavailable).
