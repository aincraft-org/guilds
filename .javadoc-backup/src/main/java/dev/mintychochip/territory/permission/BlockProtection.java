...
public final class BlockProtection {
    ...
    public boolean allowsPvp(String worldId, int blockX, int blockZ, String attackerId, String victimId) {
        ...
        Optional<Guild> guild = governance.governingGuildAt(worldId, blockX, blockZ);
    ...
    private boolean canModify(
        ...
        Optional<Guild> governing = governance.governingGuildAt(worldId, blockX, blockZ);
        if (governing.isEmpty()) {
            // Territory-local government: seat-only lockdown
            return false;
        }
        Guild guild = governing.get();
        ...
        if (body.kind() == GoverningBody.Kind.ALLIANCE) {
            Alliance alliance = body.allianceBody().orElseThrow();
            for (String guildId : alliance.memberGuildIds()) {
                Optional<Guild> sibling = governance.source().guild(guildId);
                if (sibling.isEmpty()) {
                    continue;
                }
