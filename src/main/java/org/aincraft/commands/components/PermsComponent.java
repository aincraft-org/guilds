...
/**
 * Component for checking member permissions and roles in a guild.
 */
public class PermsComponent implements GuildCommand {
    private final GuildService guildService;
    private final MemberRoleRepository memberRoleRepository;

    public PermsComponent(GuildService guildService, MemberRoleRepository memberRoleRepository) {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (!player.hasPermission(getPermission())) {
            player.sendMessage(MessageFormatter.format(MessageFormatter.ERROR, "You don't have permission to check permissions"));
            return true;
        }
        ...
        displayMemberPerms(player, guild, targetId);
        return true;
    }

    private void displayMemberPerms(Player sender, Guild guild, UUID targetId) {
        String targetName = Bukkit.getOfflinePlayer(targetId).getName();
        if (targetName == null) {
            targetName = targetId.toString();
        }

        sender.sendMessage(MessageFormatter.format(MessageFormatter.HEADER, "Permissions for " + targetName, ""));
