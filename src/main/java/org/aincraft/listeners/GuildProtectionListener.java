...
public class GuildProtectionListener implements Listener {
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        ...
        if (!canPerformAction(player, loc, GuildPermission.DESTROY)) {
            ...
            Mint.sendMessage(player, "<error>You don't have permission to break blocks here</error>");
        }
    }
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        if (!canPerformAction(player, loc, GuildPermission.BUILD)) {
            event.setCancelled(true);
            Mint.sendMessage(player, "<error>You don't have permission to place blocks here</error>");
        }
    }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (block != null) {
            ...
            if (!canPerformAction(player, loc, GuildPermission.INTERACT)) {
                event.setCancelled(true);
                Mint.sendMessage(player, "<error>You don't have permission to interact here</error>");
            }
            return;
        }
        ...
        if (!canPerformAction(player, loc, GuildPermission.BUILD)) {
            event.setCancelled(true);
            Mint.sendMessage(player, "<error>You don't have permission to use items here</error>");
        }
    }
